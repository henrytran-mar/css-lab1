# Threat model — Trang bán hàng trực tuyến

## Overview

Em chọn một trang bán hàng trực tuyến gồm ba thành phần, vì nó quen thuộc và đủ rõ để vẽ luồng dữ liệu:
...
Luồng xử lí: trình duyệt gửi yêu cầu → máy chủ ứng dụng xử lý rồi đọc/ghi cơ sở dữ liệu → trả kết quả về trình duyệt.

## Danh sách 8 mối đe dọa

Xếp theo tích tác động × khả năng, từ cao xuống thấp:

|Mã|Mối đe dọa |Nguyên lý cột sống|ATT\&CK|Tác động × Khả năng|
|-|-|-|-|-|
...
## Ba mối đe doạ tôi quyết định xử lý
...
|Mối đe doạ|Biện pháp|Chi phí dự kiến|
|-|-|-|
...
* [docs/threat-model.json](docs/threat-model.json) — mô hình theo lược đồ [schema/threat-model.schema.json](schema/threat-model.schema.json).
* [evidence/S1/preflight.txt](evidence/S1/preflight.txt) — môi trường máy, Docker daemon đang `chay`.
